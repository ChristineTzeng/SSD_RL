# always attack the other agent when they run in sight
import random
...
class AttackerAgent(Agent):
    def __init__(self, coordinates, name, agent_respawn, food_respawn, direction):
        super(AttackerAgent, self).__init__(coordinates, name, direction)
    ...
    def get_action(self, env, test = False):
        ...
        if self.is_moveout():
            ...
            return self.get_action_with_target(target)
        else:
    ...
    def get_action_with_target(self, target):
